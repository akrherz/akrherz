...
def smth9(x, p, q):
    ...
    for j in range(1, nj - 1):
        for i in range(1, ni - 1):
            ...
            ip1 = i + 1
            term1 = po4 * (
                x[im1, j] + x[i, jm1] + x[ip1, j] + x[i, jp1] - 4.0 * x[i, j]
            )
            term2 = qo4 * (
                x[im1, jp1]
                + x[im1, jm1]
...
rh_res.tmYLLabels = ["1000", "975", "950", "925", "850", "700", "500", "400"]
...
rh_res.tmXBLabelFontHeightF = 0.016  # Font size
rh_res.tmXBLabels = [
    "12z",
    "15z",
    "18z",
...
# insert a list
#
rh_res.tmXBLabelAngleF = 90.0  # change label angle
...
rain_res.trXMinF = min(taus) - dx / 2.0
rain_res.trXMaxF = max(taus) + dx / 2.0
...
px[0::5] = (taus_above_zero - dx / 2.0).astype(taus.typecode())
px[1::5] = (taus_above_zero - dx / 2.0).astype(taus.typecode())
px[2::5] = (taus_above_zero + dx / 2.0).astype(taus.typecode())
px[3::5] = (taus_above_zero + dx / 2.0).astype(taus.typecode())
px[4::5] = (taus_above_zero - dx / 2.0).astype(taus.typecode())
